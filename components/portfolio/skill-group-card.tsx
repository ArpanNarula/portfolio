  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-amber-300/20 bg-amber-300/10 text-amber-100 shadow-[0_12px_30px_rgba(217,119,6,0.14)]">
          <Icon size={20} />
        </div>
        <div>
          <p className="text-xl font-semibold text-white">{group.title}</p>
          <p className="text-sm text-stone-400">{group.subtitle}</p>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {group.items.map((item) => (
          <m.span
            ...
            className="rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-medium text-stone-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] transition group-hover:border-amber-300/18"
          >
            {item}
          </m.span>
        ))}
